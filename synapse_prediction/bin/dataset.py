...
from libs.augmentation import simpleaug_test_produce, simpleaug_test_reverse, simpleaug_train_produce, IntensityAugment
...
class SynapseDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            # 3. augmentation
            '''
            randomly select and combine augmentation method:
            simple, intensity, elastic
            '''
            if self.data_aug is not None: # augmentation
                # simple augmentation
                print ('data augmentation')
                simpaugtrain = simpleaug_train_produce()
                print ('simple augmentation')
                out_input, out_label = simpaugtrain(out_input, out_label)
                # intensity augmetation, only transform out_input
                intensityaugment = IntensityAugment()
                out_input = intensityaugment.augment(out_input)
                print ('intensity augmetation')
                print (out_input.size(),out_label.size() )
