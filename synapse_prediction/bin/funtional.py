...
def motion_blur(img):
    kernel = np.zeros((9, 9))
    xs, ys = np.random.randint(
        0, kernel.shape[1]), np.random.randint(0, kernel.shape[0])
    xe, ye = np.random.randint(
        0, kernel.shape[1]), np.random.randint(0, kernel.shape[0])
...
def random_polosa(img):
    ...
    if np.mean(gray) < 100:
        ...
        xs, ys = np.random.randint(
            0, empty.shape[1]), np.random.randint(0, empty.shape[0])
        xe, ye = np.random.randint(
            0, empty.shape[1]), np.random.randint(0, empty.shape[0])
        factor = np.random.randint(1, 10) / 3.
        cv2.line(empty, (xs, ys), (xe, ye), np.max(gray) /
                 factor, thickness=np.random.randint(10, 100))
...
def remap_color(img, bg, center, max):
    def get_lut(img, bg, center, max):
        ...
        range2 = ma - int(th)
        for i in range(3):
            channels[i].append(np.linspace(
                bg[i] - gap, center[i] - gap, int(th)).astype(np.uint8))
            channels[i].append(np.linspace(
                center[i] - gap, max[i] + gap, range2).astype(np.uint8))
            channels[i].append(
                [max[i] + gap] * (256 - sum(map(len, channels[i]))))
...
def mask_to_tensor(mask, num_classes):
    if num_classes > 1:
        ...
        mask = np.expand_dims(
            mask / (255. if mask.dtype == np.uint8 else 1), 0).astype(np.float32)
    return mask
